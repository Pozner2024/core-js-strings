...
function getFirstChar(value) {
  // Проверяем, что value является строкой и не пустая строка
  if (typeof value === 'string' && value.length > 0) {
    return value.charAt(0); // Используем charAt(0) для извлечения первого символа
  }
  return ''; // Возвращаем пустую строку, если value пустая или не строка
}
